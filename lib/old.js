...
InfluxDBConnector.prototype.create = function all(model, data, callback) {
  ...
  this.client.writePoints(model, points, {db: db}, function (err) {
    callback(err);
  });
};
...
InfluxDBConnector.prototype.all = function all(model, filter, callback) {
  ...
  if( filter.raw ) {
    this.client.queryRaw(db, query, function(err, results) {
      callback && callback( err, results );
    });
  } else if( filter.continuous ) {
    this.client.createContinuousQuery(db, 'testQuery', 'SELECT count(value) INTO valuesCount_1h FROM presence WHERE time > now() - 980h GROUP BY time(1h)', function (err, res) {
      callback && callback( err, res );
    });
  } else {
    this.client.query(db, query, function(err, results) {
      if( !err && results ) {
